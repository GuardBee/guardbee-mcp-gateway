...
const pipeline_1 = require("../gateway/pipeline");
...
function registerDbTools(server, config, db) {
    const pipeline = new pipeline_1.GatewayPipeline(config);
    // ─── Tool 1: query_table ──────────────────────────────────────────────────
    server.tool("query_table", "Query rows from a database table with optional filters. All results pass through the KVKK/GDPR gateway — sensitive fields are automatically masked.", {
        table: zod_1.z.string().describe("Table name to query"),
        filter: zod_1.z
    ...
    // ─── Tool 2: list_tables ─────────────────────────────────────────────────
    server.tool("list_tables", "List all available database tables. Tables marked as 'deny' in gateway policy are omitted.", {}, async () => {
        const allTables = await db.tables();
        const deniedTables = new Set(config.tableRules
    ...
    server.tool("describe_table", "Get the column names and gateway masking policy for a table — helps the LLM understand what data it can access.", { table: zod_1.z.string().describe("Table name") }, async ({ table }) => {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ table, columns, maxRowsPerQuery: tableRule?.maxRows ?? config.defaultMaxRows }, null, 2),
                },
            ],
        };
    });
    // ─── Tool 4: gateway_status ───────────────────────────────────────────────
    server.tool("gateway_status", "Show the active gateway configuration: field masking rules, table policies, and audit settings.", {}, async () => {
        return {
            content: [
                {
                    type: "text",
